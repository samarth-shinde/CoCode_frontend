...
export default function PlayGround({ username }) {
  ...
  const { modaltoggle, setModaltoggle } = useContext(ToolStateContext);
  ...
  const inputDefaultValue = `//Input goes here`;
  const outputDefaultValue = `//Output will show up here`;
  //ws setup
  ...
  useEffect(() => {
    axios
      .get(`${config.getPlaygroundDetailsURL}${username}`)
      .then((res) => {
  ...
  const handleInvite = () => {
    axios
      .post(`${config.inviteOthersURL}`, {
        link: `http://localhost:3000/playground/${cookieUsername["username"]}`,
        no: phoneNo,
      })
      .then((res) => {
        console.log(res);
  ...
  return (
    <>
      ...
      {helpertoggle && <Helper toggleFun={handleHelperToggle} query={query} />}
      ...
      {ws && AceComplier ? (
        <>
          <div
            className={
              modaltoggle
                ? " absolute flex justify-center mt-1 items-center inset-0 bg-black bg-opacity-30 z-10 "
                : " absolute  justify-center items-center inset-0 bg-black bg-opacity-30 z-10 hidden"
            }
          >
            <div
              id="modal"
              className=" md:w-[50vw] z-20 p-5 bg-offBlack rounded-md w-full mx-w-3xl"
            >
              ...
              <div className="form-group px-4 py-2">
                <label htmlFor="phone-no" className="block my-2 text-xl mb-4">
                  Phone No
                </label>
                <input
                  type="text"
                  id="phone-no"
                  className="w-full bg-offBlack rounded-[10px] p-2"
                  value={phoneNo}
                  onChange={(e) => {
              ...
              <div className="px-4 mt-2 mb-4" id="send-button">
                <button
                  className="p-2 bg-primary text-white w-full rounded-[12px]"
                  onClick={() => {
                    handleInvite();
                  }}
                >
          ...
          <div className=" flex justify-end my-2">
            <div className="search-bar px-3 py-2 flex items-center">
              <label className="text-2xl text-white mx-2 animate-pulse">
                <MdOutlineHelpOutline />
              </label>
              <input
                type="text"
                className="bg-offBlack rounded-[5px] p-2"
                placeholder="Type Your Query.."
                value={query}
                onChange={(e) => setQuery(e.target.value)}
              />
              <button
                className="bg-transparent border-2 border-blue-600 p-2 text-offWhite rounded-[5px] ml-1 hover:text-white hover:shadow-sm hover:shadow-blue-500/50"
                onClick={() => setHelpertoggle(true)}
              >
                Search
              </button>
            </div>
          ...
          <div className=" w-full px-px ">
                ...
                <div>
                  ...
                  >
                    <h3 className="flex justify-center items-center text-base mx-3">
                      {writingUser} is currently wants to add to this code
            ...
            <div className="flex w-full px-2 md:px-6 justify-center  mt-1">
              <div className="w-full">
                ...
                {
                  //this is console and iske niche jo do ace edit liha hena they are input n output console
                }
                <AceComplier
                ...
                <div className="grid grid-cols-2 md:grid-cols-2 gap-0 mt-5 mb-5">
                  <AceEdit
                    mode={"markdown"}
                    theme={theme}
